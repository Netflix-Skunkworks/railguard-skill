...
Use these tools to find concrete evidence for gate evaluation:

**ast-grep** (`ast-grep run -p '<PATTERN>' -l <LANG> --json {{REPO_PATH}}`): Structural
code search that matches AST nodes, not text. Unlike grep, it ignores matches in
comments and strings. Use `$VAR` for single-node metavariables, `$$$` for multi-node.
Examples for gate evaluation:
- `ast-grep run -p 'pickle.loads($$$)' -l python --json {{REPO_PATH}}` — confirm serialization
- `ast-grep run -p 'render_template_string($$$)' -l python --json {{REPO_PATH}}` — confirm SSTI surface
- `ast-grep run -p '$X.execute($$$)' -l python --json {{REPO_PATH}}` — confirm SQL execution
- `ast-grep run -p '_.merge($$$)' -l js --json {{REPO_PATH}}` — confirm deep merge patterns

**LSP** (if status is `full` or `partial`):
- **`findReferences`** to confirm an imported symbol is actually called in non-test code
- **`hover`** to disambiguate when the same function name exists in multiple libraries

**grep/rg**: Use for text searches (config values, dependency names, string patterns).

Prefer ast-grep over grep for gate evaluation because the gate criteria require
"concrete evidence in actual code -- not in comments." ast-grep enforces this by design.
